...
static uint32_t ImGui_ImplVulkan_MemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
    ...
    for (uint32_t i = 0; i < prop.memoryTypeCount; i++)
        if ((prop.memoryTypes[i].propertyFlags & properties) == properties && type_bits & (1u << i))
            return i;
    return UINT32_C(0xffffffff); // Unable to find memoryType
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        enum : size_t { nonCoherentAtomSize = 64 };
        // Create or resize the vertex/index buffers
        size_t vertex_size = (size_t)draw_data->TotalVtxCount * sizeof(ImDrawVert);
        vertex_size = quickgui::next_multiple<size_t, nonCoherentAtomSize>(vertex_size);
        size_t index_size = (size_t)draw_data->TotalIdxCount * sizeof(ImDrawIdx);
        index_size = quickgui::next_multiple<size_t, nonCoherentAtomSize>(index_size);
    ...
    unsigned int global_idx_offset = 0;
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        const ImDrawList* cmd_list = draw_data->CmdLists[n];
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < (float)fb_width && clip_rect.y < (float)fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    // Draw
                    vkCmdDrawIndexed(command_buffer, pcmd->ElemCount, 1, pcmd->IdxOffset + global_idx_offset, (int)pcmd->VtxOffset + global_vtx_offset, 0);
                }
            }
        }
        global_idx_offset += (decltype(global_idx_offset)) cmd_list->IdxBuffer.Size;
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    size_t upload_size = (size_t)(width * height * 4) * sizeof(char);
    ...
    VkDescriptorSet font_descriptor_set = ImGui_ImplVulkan_AddTexture(g_FontSampler, g_FontView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
VkSurfaceFormatKHR ImGui_ImplVulkanH_SelectSurfaceFormat(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkFormat* request_formats, int request_formats_count, VkColorSpaceKHR request_color_space)
{
    ...
    else
    {
        ...
        for (int request_i = 0; request_i < request_formats_count; request_i++)
            ...
            {
                auto af = avail_format[(int)avail_i];
                if (af.format == request_formats[request_i] && af.colorSpace == request_color_space)
                    return af;
            }
...
VkPresentModeKHR ImGui_ImplVulkanH_SelectPresentMode(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkPresentModeKHR* request_modes, int request_modes_count)
{
    ...
    for (int request_i = 0; request_i < request_modes_count; request_i++)
        for (uint32_t avail_i = 0; avail_i < avail_count; avail_i++)
            if (request_modes[request_i] == avail_modes[(int)avail_i])
...
void ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator)
{
    ...
    for (uint32_t i = 0; i < wd->ImageCount; i++)
    {
        ...
        }
        {
            ...
            C4_CHECK(&fd->CommandBuffer + 1 == &fd->CommandBuffer2);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    if (min_image_count == 0)
        min_image_count = (uint32_t)ImGui_ImplVulkanH_GetMinImageCountFromPresentMode(wd->PresentMode);
    ...
    // Create Swapchain
    {
        ...
        {
            wd->Width = (int)cap.currentExtent.width;
            wd->Height = (int)cap.currentExtent.height;
        }
        info.imageExtent.width = (uint32_t)wd->Width;
        info.imageExtent.height = (uint32_t)wd->Height;
    ...
    // Create Framebuffer
    {
        ...
        info.attachmentCount = 1;
        info.pAttachments = attachment;
        info.width = (uint32_t)wd->Width;
        info.height = (uint32_t)wd->Height;
        info.layers = 1;
        for (uint32_t i = 0; i < wd->ImageCount; i++)
        {
            ImGui_ImplVulkanH_Frame* fd = &wd->Frames[i];
...
void ImGui_ImplVulkanH_DestroyFrame(VkDevice device, ImGui_ImplVulkanH_Frame* fd, const VkAllocationCallbacks* allocator)
{
    ...
    C4_ASSERT(&fd->CommandBuffer + 1 == &fd->CommandBuffer2);
    vkFreeCommandBuffers(device, fd->CommandPool, 2, &fd->CommandBuffer);
    vkDestroyCommandPool(device, fd->CommandPool, allocator);
    fd->Fence = VK_NULL_HANDLE;
    fd->CommandBuffer = VK_NULL_HANDLE;
    fd->CommandBuffer2 = VK_NULL_HANDLE;
